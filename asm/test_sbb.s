...
_start:
    ...
    sub al, bl ; this generates a carry
    ...
    sbb al, bl ; use the carry

    mov BYTE [C + 1], al ; save the second byte

    mov al, 0 ; clear al

    sbb al, 0 ; get the carry (there is none)
...
check_byte_1:
    cmp BYTE [C], 11010100b
    je check_byte_2
    jmp fail

check_byte_2:
	cmp BYTE [C + 1], 00101101b
	je check_byte_3
	add BYTE [message_fail + 5], 1
	jmp fail

check_byte_3:
	cmp BYTE [C + 2], 00000000b
	je success
...
section .bss
	C: resb 3 ; the result of (A - B) is 00000000 00101101 11010100 == 11732

section .data
	A: dw 1010101011001011b ; 43723
	B: dw 0111110011110111b ; 31991

    message_1 db "Testing sbb...",10,"Subtracting two large numbers...", 10
